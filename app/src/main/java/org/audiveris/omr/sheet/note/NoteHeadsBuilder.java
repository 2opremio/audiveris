...
public class NoteHeadsBuilder
{
     ...
     * <p>
     * Not to keep heads off them, which would settle the competition a barline has with a
     * stem before it is fought: a barline that is only a candidate has to be free to lose
     * to a real note. Only to withhold the slack a stroke is granted, so that a head there
     * has to be as convincing as it was before the slack existed.
     *
    ...
    private class Scanner
    {
                                    ...
                                    Anchor anchor)
        {
            ...
            // No slack where a barline is: its ink reads as a stem, and the digits of
            // the time signature beside it then read as the heads hanging off it. A head
            // there must be as convincing as it was before the slack existed.
        ...
        //----------------//
        /**
         * Report whether the ink a head was built from is as tall as the template that
         * found it.
         * <p>
         * A template reports a distance and the glyph underneath reports its own bounds, and
         * the two part company where the match is on something else: the numeral above a
         * measure repeat sign reads as a cross head half a head tall, and a head's ink is
         * erased before the symbol step looks, so the sign goes with it. Over a library of 95
         * drum charts 99.8% of heads stand a full template high and the ones that do not are
         * this.
         *
